...
const toString =
  (trail: Hashes, path: Path, size: number, engine: string) => (): string =>
    Buffer.from(
      `${Buffer.concat(trail).toString("hex")}.${path}.${engine}.${size}`
    ).toString("base64");
...
export const merkleProofFrom = (str: string): MerkleProof => {
  try {
    ...
    switch (engine) {
      case SHA_256:
        const hashes = parts[0]
          .split(/(.{64})/)
          .filter((_) => _.length == 64)
          .map((_) => Buffer.from(_, "hex")); // eslint-disable-line no-case-declarations
